...
pyautogui.doubleClick()  # Double click to select the conversion rate
pyautogui.hotkey('ctrl', 'c')  # Copy the selected conversion rate
time.sleep(3)
...
#pyautogui.hotkey('ctrl', 'n')  # Open a new Excel workbook
#time.sleep(2)
pyautogui.typewrite(datetime.now().strftime("%Y-%m-%d %H-%M-%S"), interval=0.1)  # Type the current date and time
pyautogui.press('tab')  # Move to the next cell
pyautogui.hotkey('ctrl', 'v')  # Paste the copied conversion rate
pyautogui.press('tab')  # Move to the next cell
pyautogui.press('tab')  # Move to the next cell
pyautogui.typewrite("Conversion rate captured Successfully", interval=0.1)  

pyautogui.hotkey('ctrl', 's')  # Save the Notepad file
time.sleep(2)
pyautogui.typewrite(file_path, interval=0.1)  # Type the Notepad file path
pyautogui.press('enter')
